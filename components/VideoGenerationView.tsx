...
import { SendIcon, VideoIcon, UploadIcon, CameraIcon, TrashIcon, MenuIcon } from './Icons';

interface VideoGenerationViewProps {
  activeModel: AIModel;
  onGenerateVideo: (prompt: string, inputImage: { data: string; mimeType: string; } | null) => void;
  isLoading: boolean;
  error: string | null;
...
const VideoGenerationView: React.FC<VideoGenerationViewProps> = ({ activeModel, onGenerateVideo, isLoading, error, generatedVideoUrl, progressMessage, inputImage, setInputImage, onToggleSidebar }) => {
  ...
  const handleGenerate = () => {
    if (prompt.trim() && !isLoading) {
      onGenerateVideo(prompt, inputImage);
    }
  };
  ...
  const InputSourceSelector = () => (
    <div className="text-center text-slate-500">
         ...
         <h3 className="text-2xl font-semibold text-slate-300">Animate your imagination</h3>
         <p className="mb-6 max-w-md mx-auto">Start with a text prompt to create a new video, or upload an image to bring it to life.</p>
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-900">
      <header className="flex items-center p-4 border-b border-slate-800">
        ...
        <h2 className="text-xl font-semibold">{activeModel.name} Video Generation</h2>
      </header>
      ...
      <div className="flex-1 overflow-y-auto p-6 flex items-center justify-center">
        ...
        {isLoading ? (
          <div className="w-full max-w-lg aspect-video bg-slate-800 rounded-lg flex flex-col items-center justify-center text-center p-4">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-400 mb-4"></div>
            <p className="text-slate-300 font-semibold text-lg">Generating your video...</p>
            <p className="text-slate-400 text-sm">{progressMessage || "This may take a few minutes."}</p>
          </div>
        ) : error ? (
           <div className="w-full max-w-lg aspect-video bg-slate-800/50 border-2 border-dashed border-red-500/50 rounded-lg flex flex-col items-center justify-center text-center p-4">
        ...
        <div className="relative">
          <textarea
            ...
            placeholder={inputImage 
                ...
                : `Describe a video, e.g., "A majestic lion roaring on a rocky outcrop at sunset"`
            }
